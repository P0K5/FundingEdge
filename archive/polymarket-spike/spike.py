...
from polymarket_client import get_weather_markets, get_orderbook


# Rough fee approximation for the spike.
# Polymarket taker fee is ~2% of order value; this approximation is good enough
# for candidate flagging. The full build needs the real schedule.
...
# Polymarket city name (lowercase) -> METAR station code, derived from STATIONS
...
def _enrich_from_clob(bracket: Bracket) -> None:
    """Overwrite bracket ask prices with live CLOB data (per-token book lookup)."""
...
def poll_once():
    ...
    # 2. Fetch Polymarket weather-tagged markets and scan
    try:
        all_markets = get_weather_markets()
    except Exception as e:
        print(f"[polymarket] error fetching markets: {e}. Skipping this poll.")
        return
    ...
    for market in all_markets:
        try:
            ...
            if candidate:
                ...
                print(f"  ** FLAGGED {bracket.ticker[:16]}… {side} @ {price}¢ edge={edge:.2f}¢ p={confidence:.2%}")
    ...
    print(f"[scan] {len(all_markets)} fetched, {n_temp} highest-temp matched, "
          f"{n_brackets} brackets parsed, {n_flagged} flagged")


def main():
    print("MeteoEdge-Polymarket spike starting. Observe-only mode.")
